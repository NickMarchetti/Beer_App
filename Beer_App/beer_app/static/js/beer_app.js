...
var thead = d3.select("thead");  //Store the header of the table
...
function filterVotes(event) {
    ...
    d3.selectAll("tbody tr").remove(); //remove any data currently in the table
    // tableHeaders();  //add table headers back in
    response.forEach(function(beer) {
...
$(function () {
    var stateList = [];
    response.forEach(function(beer) {
          if (!(stateList.includes(beer.brewery_state))) {
            stateList.push(beer.brewery_state);
          }
    });  //end filterName function
    $("#state").autocomplete({
        source:stateList
