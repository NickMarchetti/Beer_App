        ...
        click: function(event) {
          ...
          d3.json(link).then(function(stuff) {
            ...
            for (var j = 0; j < 1000; j++) {
              if (stateAbr == response[j].brewery_state) {
                if (!(breweries.includes(response[j].brewery))) {
                  breweries.push(response[j]);
                  console.log("Adding: ", response[j].brewery);
                }
              }
            }
            for (var i = 0; i < breweries.length; i++) {
              L.marker([breweries[i].brewery_latitude, breweries[i].brewery_longitude])
               .bindTooltip(breweries[i].brewery)
               .addTo(map);
            }
          });
        },
